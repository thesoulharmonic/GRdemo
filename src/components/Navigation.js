...
function Navigation() {
  ...
  // handleToggleNotifications that toggles the display of the notification element and sends a POST request to update

  function handleToggleNotifications() {
    const position = bellRef.current.getBoundingClientRect();
    setBellPos(position);
    // toggle the display of the notification element

    notificationRef.current.style.display =
      notificationRef.current.style.display === "block" ? "none" : "block";
    dispatch(resetNotifications());
    // if there are any unread notifications, send a POST request to update
  ...
  return (
    <Navbar bg="light" expand="lg">
      <Container>
        <LinkContainer to="/">
          <Navbar.Brand>
            <img src="../images/GRlogo.png" width="46px" height="46px" />
          </Navbar.Brand>
        </LinkContainer>
