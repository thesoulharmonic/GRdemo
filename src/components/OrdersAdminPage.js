...
function OrdersAdminPage() {
    ...
    function markShipped(orderId, ownerId) {
        // send a PATCH request to mark an order as shipped with the given ID and owner ID
        axios.patch(`/orders/${orderId}/mark-shipped`, { ownerId })
            .then(({ data }) => setOrders(data))  // Update the orders state 
            .catch((e) => console.log(e));        // Log any errors to the console
    }
    
    function showOrder(productsObj) {
        // filter the products based on the IDs in the given products object and update the count for each one
        ...
        // log the products to be shown and update the order to show in the state.
